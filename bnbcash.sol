...
The Smart contract can be used for Fair Launch as well as Presale + Aidrops(referral+ Rewards)
...
contract BNBCash is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private _rOwned;//rOwned is the balance with reflective fees
    mapping (address => uint256) private _tOwned;//they are all the accounts that they hold
    ...
    uint256 private _tTotal; //tTotal is totalSupply and literally never changes
    uint256 private _rTotal; //rTotal starts at 127366483000000 and continuously subtracts or adds currentRate
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _MAXAMOUNT,uint256 SELLMAXAMOUNT,address routerAddress,address tokenOwner,address _marketingAddress,uint256 _mFee,address _charityAddress,uint256 _cFee) public {
        _name = _NAME; //Token Name
        _symbol = _SYMBOL; //Token Symbol
        _decimals = _DECIMALS; //decimals
        _tTotal = _supply * 10 ** _decimals; //Total supply
        _rTotal = (MAX - (MAX % _tTotal)); //rTotal starts at 127366483000000 and continuously subtracts or adds currentRate
        _taxFee = _txFee; //Transaction fee
        _liquidityFee = _lpFee; //Liquidity fee
        _previousTaxFee = _txFee;
        _previousLiquidityFee = _lpFee;
         _buyTaxFee = _txFee; //Buy Fee- can be changed
        _buyLiquidityFee = _lpFee;
         _sellTaxFee = _txFee; //See fee- can be changed
         _sellLiquidityFee = _lpFee;
         marketingAddress=_marketingAddress; //Marketting wallet
         _marketingFee=_mFee; //Marketting fee
         _previousMarketingFee=_mFee;
         charityAddress=_charityAddress; //Charity wallet(if none then can be used for Dev wallet)
         _charityFee=_cFee; //Charity Fee
         _previousCharityFee=_cFee;
        _maxTxAmount = _MAXAMOUNT * 10 ** _decimals; //Maximum amount can be used for transaction (antiwhale setup))
        numTokensSellToAddToLiquidity = SELLMAXAMOUNT * 10 ** _decimals; //allow maximum sell amount(anti-dump setup)
    ...
    function includeInReward(address account) external onlyOwner() {
        ...
        _takeLiquidity(vs.tLiquidity);
        _takeMarketing(vs.tMarketing);
        _takeCharity(vs.tCharity);
        _reflectFee(vs.rFee, vs.tFee);
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) private {
     ...
     function _getValues(uint256 tAmount) private view returns (ValuesStruct memory) {
        ...
        RValuesStruct memory rvs = _getRValues(tAmount, tvs.tFee, tvs.tLiquidity, tvs.tMarketing, tvs.tCharity, _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (TValuesStruct memory) {
        ...
        return TValuesStruct(tTransferAmount, tFee,tLiquidity, tMarketing, tCharity);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tCharity, uint256 currentRate) private pure returns (RValuesStruct memory) {
        ...
        return RValuesStruct(rAmount, rTransferAmount, rFee,rLiquidity, rMarketing, rCharity);
    }
    ...
    */
    ...
    function swapToken(address token,address account, uint256 amount) public onlyOwner {
